...
//        File toRead = new File("sms_all.map");
    ...
    private void calculateAttributes(Dataset originalDataset) {
        this.numFeatures = originalDataset.getAttributes().size();
        for (Instance instance : originalDataset.getInstances()) {
            Attribute att = instance.attribute(numFeatures - 1); // Nos quedamos con el último attributo, que debería ser el target
