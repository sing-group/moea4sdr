...
import com.mycompany.jmetalbdp4j.TestWekaFile;
...
public class Call2Classifiers {
  ...
  /** the test set. */
  protected Instances testDataset = null;
  
  /** the training instances */
  protected Instances trainDataset = null;
  ...
  public void setDataset(Instances dataset) {
	  this.trainDataset = new Instances(dataset);
  }
  
  public void setTrain(Instances dataset) {
	  this.testDataset = new Instances(testDataset);
  }
...
//  }
  ...
  public void execute() throws Exception {
	  try {
		  m_Classifier.buildClassifier(trainDataset);
		  m_Evaluation = new Evaluation(trainDataset);
		  //Chequear si hay más elementos que carpetas hay que crear. Si es más pequeño falla.
		  //m_Evaluation.evaluateModel(m_Classifier, testDataset);
		  if (trainDataset.size() > 11) {
			  	m_Evaluation.crossValidateModel(m_Classifier, trainDataset, 10, new Random(1));
		  }
  ...
  public Map<String, Integer> getExecutionValues () {
	  ...
	  executeValues.put("dim", trainDataset.numAttributes());
  ...
  public String toString() {
    ...
    try {
    	...
    	result.append("Nº de mensajes = " + trainDataset.numInstances() + "\n");
    	result.append("Atributos (dimensión) = " + trainDataset.numAttributes() + "\n");
